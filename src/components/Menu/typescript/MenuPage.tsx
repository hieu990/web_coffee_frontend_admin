...
import MenuBook, { PageData } from './MenuBook';
// @ts-ignore
...
export default function MenuPage() {
  ...
  // Ordering state
  const [orderItems, setOrderItems] = useState<Array<MenuItem & { qty: number }>>([]);
  const [isTrayExpanded, setIsTrayExpanded] = useState<boolean>(false);

  // Construct dynamic pages array from menuData categories
  const pages: PageData[] = useMemo(() => {
    const rawPages: PageData[] = [
      { type: 'cover' },
      ...Object.keys(menuData).flatMap((key) => [
        { 
          type: 'intro' as const, 
          categoryKey: key, 
          title: menuData[key].title, 
          intro: menuData[key].intro, 
          image: menuData[key].image 
        },
        { 
          type: 'items' as const, 
          categoryKey: key, 
    ...
    // If total page count is odd, insert a blank page right before the back cover to balance layout
    if (rawPages.length % 2 !== 0) {
      rawPages.splice(rawPages.length - 1, 0, { type: 'blank' });
    }
